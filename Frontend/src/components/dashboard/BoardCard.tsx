...
export const BoardCard: React.FC<BoardCardProps> = React.memo(({
  ...
  const totalTasks = board.stats?.totalTasks ?? 0;
  const doneCount = board.stats?.doneCount ?? 0;
  const progressPercent = totalTasks > 0 ? Math.round((doneCount / totalTasks) * 100) : 0;
  const tags = board.tags || [];
  const members = board.members || [];
  const color = board.color || 'from-indigo-600 to-violet-600';
  const workspaceName = board.workspaceName || 'Workspace';
